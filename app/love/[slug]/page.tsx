...
import { loveArticles, getLoveArticle } from '@/lib/love';

const LINE_URL = 'https://lin.ee/iweaTucb';
const LINE_ID = '@datenami';

export async function generateStaticParams() {
  return loveArticles.map((a) => ({ slug: a.slug }));
}
...
export async function generateMetadata({
  ...
  const article = getLoveArticle(slug);
  if (!article) return {};
  return {
    title: article.title,
...
export default async function LoveArticlePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const related = article.related
    .map((s) => getLoveArticle(s))
    .filter((a): a is NonNullable<typeof a> => !!a);
  const others = related.length > 0
    ? related
    : loveArticles.filter((a) => a.slug !== slug).slice(0, 3);
        ...
        <div className="mx-auto w-full max-w-[900px] px-6 pt-8 sm:px-8 lg:px-10">
          <div className="relative overflow-hidden rounded-2xl">
            ...
            <div className="absolute bottom-0 left-0 right-0 px-6 pb-6 sm:px-8">
              ...
              <p className="mt-2 text-white/75 text-sm">{article.date}</p>
            </div>
          </div>
        </div>
